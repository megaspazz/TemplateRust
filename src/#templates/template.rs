...
const TEST_TYPE: TestType = TestType::Single;
const START_TEST_CASE: usize = 1;
...
impl<'s> Input<'s> {
	...
	pub fn read_line_vec(&mut self, size: usize) -> Vec<String> {
        (0..size).map(|_| self.read_line()).collect()
	}
	...
	pub fn read_string_vec(&mut self, size: usize) -> Vec<String> {
        (0..size).map(|_| self.read_string()).collect()
	}
	...
	read_impl!(f64, read_f64, read_f64_vec);
...
impl Output {
	...
	pub fn new(output: Box<dyn Write>) -> Self {
		Self {
			...
			at: 0,
			auto_flush: false,
			bool_output: BoolOutput::TrueFalse,
		}
	}
